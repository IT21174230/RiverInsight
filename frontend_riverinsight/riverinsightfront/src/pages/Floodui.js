...
export default function FloodDashboard() {
  ...
  const fetchPrediction = (date) => {
    setLoading(true);
    fetch(`http://localhost:8000/predict/flooding?date=${date}`)
      .then((res) => res.json())
      .then((data) => {
        setDashboardData({
          date: data.date,
      ...
      <div className={`${cardBg} w-full max-w-6xl rounded-3xl p-8 shadow-lg`}>
        ...
        {Object.keys(xaiFeatureImportance).length > 0 && (
          <div className="mt-10 p-6 rounded-xl shadow-md border bg-white dark:bg-gray-800">
            <h3 className="text-lg font-semibold text-blue-600 mb-4">XAI Feature Importance</h3>
              ...
              <tbody>
                {Object.entries(xaiFeatureImportance)
                  ...
                  .map(([feature, importance], index) => (
                    ...
                    >
                      <td className="py-2 text-gray-800 dark:text-gray-100">{feature}</td>
                      <td className={`py-2 ${importance >= 0 ? "text-green-500" : "text-red-400"}`}>
